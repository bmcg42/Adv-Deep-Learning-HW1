...
class Linear4Bit(torch.nn.Module):
    ...
    def _load_state_dict_pre_hook(
        ...
        if f"{prefix}weight" in state_dict:
            ...
            self.weight_q4, self.weight_norm = block_quantize_4bit(torch.flatten(weight))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            # TODO: Dequantize and call the layer
            # Hint: You can use torch.nn.functional.linear
            weight_dq = block_dequantize_4bit(self.weight_q4,self.weight_norm)
            weight_dq = weight_dq.reshape(self._shape) # W: (out_features, in_features)
            return torch.nn.functional.linear(x,weight_dq,self.bias)
